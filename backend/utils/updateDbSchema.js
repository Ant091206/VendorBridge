...
async function runSchemaUpdate() {
  ...
  try {
    ...
    await checkAndAddColumn(connection, 'activity_logs', 'module', 'VARCHAR(100) NULL AFTER role');
    await checkAndAddColumn(connection, 'activity_logs', 'description', 'TEXT NULL AFTER action');
    ...
    await checkAndAddIndex(connection, 'activity_logs', 'idx_activity_logs_module', '`module`');
    await checkAndAddIndex(connection, 'activity_logs', 'idx_activity_logs_user_id', '`user_id`');
    
    await checkAndAddIndex(connection, 'notifications', 'idx_notifications_user_unread', '`user_id`, `is_read`, `created_at` DESC');
    
    await checkAndAddIndex(connection, 'vendors', 'idx_vendors_email_status', '`email`, `status`');
    await checkAndAddIndex(connection, 'rfqs', 'idx_rfqs_status_deadline', '`status`, `deadline`');
    ...
    await checkAndAddIndex(connection, 'invoices', 'idx_invoices_issued', '`po_id`, `status`, `issued_at` DESC');
