  ...
  return (
    <div className="space-y-6 max-w-5xl mx-auto pb-10">
      ...
      <div className="border-b border-slate-800 pb-4">
        <h1 className="text-2xl font-bold text-white tracking-tight leading-normal">
          ...
          <div className="rounded-xl border border-slate-800 bg-slate-900 p-6 space-y-4">
            ...
            {!isLowest && (
              <div className="rounded-lg bg-amber-500/10 border border-amber-500/25 p-3.5 flex gap-3 text-amber-400 text-xs leading-relaxed">
                ...
                <div>
                  <span className="font-bold">Aesthetic Price Warning:</span> This quotation is <strong>{percentage_difference}%</strong> higher than the lowest submitted offer (which was {formatCurrency(lowest_price)}). Ensure vendor specifications or delivery guidelines justify the cost delta.
                </div>
              </div>
            )}
                ...
                {(user?.role === 'manager' || user?.role === 'admin') ? (
                  <>
                    <button
                      ...
                      className="w-full py-3 px-4 rounded-lg bg-emerald-600 hover:bg-emerald-500 text-white font-bold text-sm transition shadow-lg shadow-emerald-600/10 text-center"
                    >
                      Approve Request
                    </button>
                    <button
                      onClick={() => setShowRejectModal(true)}
                      className="w-full py-3 px-4 rounded-lg bg-rose-600/10 hover:bg-rose-600 text-rose-400 hover:text-white font-bold text-sm border border-rose-500/20 hover:border-transparent transition text-center"
                    >
                      Reject Request
                    </button>
                  </>
